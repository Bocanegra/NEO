    ...
    window.iui =
{
    ...
    //This function runs only when the pages are changes via a normal button,
    //not the back button, so I have set it to allways say false for backwards
...
addEventListener("click", function(event) 
{
        ...
        if (link) {
            ...
            if (link.href && link.hash && link.hash != "#") {
                ...
                //This statment runs everytime the back button is clicked
                //(but not the normal buttons) so we allways send true for
                //backwards

                var backButton = $("backButton");
                backButton.style.opacity = 0;

                iui.showPage($(link.hash.substr(1)), true);

                //setTimeout(unselect, 500);
                unselect();
                backButton.style.display = "none";
            }
            //This causes our program to crash so we have redone
            //the back button a different way
            ...
            else if (link.getAttribute("disablePreventDefault") == "true") {
                return;
            }
...
addEventListener("click", function(event)
{
    ...
    function updatePage(page, fromPage) {
        ...
        */
        ...
        //Cole Added this code to handel the back button with manual control
        var backButton = $("backButton");
        if (backButton) {
            var backButtonValue = page.getAttribute("backButtonLabel");
            if (backButtonValue) {
                backButton.innerHTML = backButtonValue;
                backButton.href = page.getAttribute("backButtonHref");
                backButton.className = "button " + page.getAttribute("backButtonClass");
                backButton.style.display = "inline";
                setTimeout("backButton.style.opacity = 1;", 1);
            }
            else
                backButton.style.display = "none";
        }

        //Cole Added this code to handel the logoff button with manual control
