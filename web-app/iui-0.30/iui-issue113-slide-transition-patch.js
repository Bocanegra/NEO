...
var slideTransitionDuration = 900;
...
{        
    ...
    // Now we need to animate it to the desired position
    fromPage.style.webkitTransitionProperty = transitionProperty;
    fromPage.style.webkitTransitionDuration = slideTransitionDuration + 'ms';
    fromPage.style.transitionTimingFunction = 'default';

    toPage.style.webkitTransitionProperty = transitionProperty;
    toPage.style.webkitTransitionDuration = slideTransitionDuration + 'ms';
    toPage.style.transitionTimingFunction = 'default';

    slide(0);

    // Call completion function sometime after we think we'll be done
    setTimeout(slideComplete, slideTransitionDuration);

    function slide(pct)
    {
        if (axis == "y") {
           if (backwards) {
              fromPage.style.top = (100-pct) + "%"
           } else {
               toPage.style.top = pct + "%";
           }
        } else {
           var fromLeft = (backwards ? (100-pct) : (pct-100)) + "%";
           var toLeft = (backwards ? -pct : pct) + "%";
           fromPage.style.left = fromLeft; 
           toPage.style.left = toLeft; 
        }
    }
